...
int main(){
    ...
    if (rank == 0){
        ...
        position = 0;
        MPI_Pack(&data.a, 1, MPI_INT, buffer, buffer_size, &position, MPI_COMM_WORLD);   // initial position is 0, position automatically increments
        ...
        // These 2 lines for getting to know the 'buffer_size' that is being sent
        ...
        // unpack in the same order as packing is done    
        position = 0;
